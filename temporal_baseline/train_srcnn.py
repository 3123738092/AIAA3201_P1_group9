"""Train SRCNN on Vimeo-90K or REDS (single-frame super-resolution).

Usage:
    # Vimeo-90K
    python train_srcnn.py --dataset vimeo --data_root /path/to/vimeo_septuplet

    # REDS
    python train_srcnn.py --dataset reds --data_root /path/to/reds
"""
...
from datasets.reds import REDSDataset
...
def build_datasets(args):
    if args.dataset == 'vimeo':
        train_set = Vimeo90KSeptuplet(args.data_root, split='train',
                                       scale=args.scale, patch_size=args.patch_size)
        test_set = Vimeo90KSeptuplet(args.data_root, split='test', scale=args.scale)
    else:  # reds
        train_set = REDSDataset(args.data_root, split='train', scale=args.scale,
                                num_frames=args.num_frames, patch_size=args.patch_size)
        test_set = REDSDataset(args.data_root, split='val', scale=args.scale,
...
def train_one_epoch(model, loader, criterion, optimizer, device, scale):
    ...
    for batch in tqdm(loader, desc='Train', leave=False):
        lr = batch['lr'].to(device)   # (B, T, 3, h, w)
        gt = batch['gt'].to(device)   # (B, T, 3, H, W)
        B, T, C, H, W = gt.shape
        h, w = lr.shape[3], lr.shape[4]

        # Bicubic upsample LR, then feed to SRCNN (per-frame)
        lr_up = torch.nn.functional.interpolate(
            lr.view(B * T, C, h, w), size=(H, W), mode='bicubic', align_corners=False
        )
